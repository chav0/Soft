...
namespace Client
{
    public class GameLogic
    {
        ...
        public World CreateNewWorld(int worldId)
        {
            ...
            var world = new World(worldObject, worldId);
            return world;
        }
            ...
            for (var i = 0; i < _world.Field.Count; i++)
            {
                ...
                for (var j = row.Count - 2; j >= 0; j--)
                {
                    ...
                    if (toCell.GameCell != null)
                        MergeCell(fromCell, toCell, fromCell.GameCell, toCell.GameCell);
                    else
        ...
        private void MergeCell(FieldCell from, FieldCell to, GameCell deletedCell, GameCell mergedCell)
        {
            deletedCell.NeedDelete = true;
            deletedCell.FieldCell = to;
            mergedCell.MergeCount++; 
            from.GameCell = null;
